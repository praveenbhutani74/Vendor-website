      ...
      <div className="min-h-screen flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center bg-gray-50 py-20">
          ...
          >
            ...
            <h2 className="text-2xl font-bold text-[#0c1c5e] mb-3">Quote Request Sent!</h2>
            ...
            <Link href="/materials">
              <button className="bg-[#1a4fdb] text-white font-semibold px-8 py-3 rounded-full hover:bg-[#0c1c5e] transition-colors">
                Continue Shopping
              </button>
            </Link>
          </motion.div>
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="flex-1 bg-gray-50 py-12">
        <div className="container mx-auto px-4 max-w-5xl">
          <Link href="/materials" className="inline-flex items-center gap-2 text-[#1a4fdb] text-sm font-medium mb-8 hover:gap-3 transition-all">
          ...
          {items.length === 0 ? (
            <div className="bg-white rounded-2xl p-16 text-center border border-gray-100 shadow-sm">
              ...
              <h3 className="text-xl font-bold text-[#0c1c5e] mb-3">Your cart is empty</h3>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-4">
                <div className="flex items-center justify-between mb-2">
                  <h2 className="font-bold text-[#0c1c5e] text-lg">{total} item{total !== 1 ? "s" : ""} in cart</h2>
                ...
                <AnimatePresence>
                  {items.map((item) => (
                    ...
                    >
                      <img src={item.image} alt={item.productName} className="w-20 h-20 object-cover rounded-lg shrink-0" />
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-gray-400 uppercase tracking-wide">{item.categoryName}</p>
                        <p className="font-bold text-[#0c1c5e] mt-0.5">{item.productName}</p>
              ...
              <div className="lg:col-span-1">
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sticky top-24">
                  <h3 className="font-bold text-[#0c1c5e] text-lg mb-5 flex items-center gap-2">
                    <Send className="w-5 h-5 text-[#1a4fdb]" /> Submit Quote Request
                  </h3>
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                      <label className="text-xs font-semibold text-gray-600 block mb-1">Full Name *</label>
                      <input required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                        className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#1a4fdb]/30 focus:border-[#1a4fdb]"
                    ...
                    <div>
                      ...
                      <textarea value={form.message} onChange={e => setForm(f => ({ ...f, message: e.target.value }))}
                        rows={3}
                        className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#1a4fdb]/30 focus:border-[#1a4fdb] resize-none"
                        placeholder="Delivery location, project timeline, special requirements..."
                      />
                    </div>
                    <button type="submit" className="w-full bg-[#1a4fdb] hover:bg-[#0c1c5e] text-white font-bold py-3.5 rounded-xl transition-colors flex items-center justify-center gap-2">
